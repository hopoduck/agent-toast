...
- **클릭 한 번**으로 알림을 띄운 그 터미널 창을 바로 활성화
- 터미널로 돌아오면 **알림 자동 소멸**, 이미 보고 있는 터미널이면 알림 생략
- 알림 본문에 **에이전트의 마지막 메시지** 표시 (권한 요청 시 도구 설명)
- 작업 완료·권한 요청·입력 대기·세션 시작/종료 등 **15가지 훅 이벤트**
- **원격 Linux 서버**의 Claude Code 훅 알림을 데스크톱 토스트로 수신
- **멀티 모니터** 4코너 중 원하는 위치에 표시, DPI 스케일 대응
- **시스템 알림음**으로 이벤트 놓침 방지 (설정에서 on/off)
- 시스템 **라이트/다크 테마** 연동, 마우스를 올리면 자동 닫힘 일시정지
- 바·테두리·배경·이펙트·밀도(넉넉/컴팩트)와 sans/mono 시스템 폰트를 **실시간 미리보기**로 조정 (D2Coding 번들 포함)
- 표시·클릭·자동 소멸 등 알림 이벤트를 집계한 **통계 인사이트**
- **한국어/영어** UI
- 새 버전 알림과 **원클릭 자동 업데이트**
...
- 최초 실행 시 앱을 띄우고, 이후 CLI 호출은 Named Pipe로 JSON만 보낸 뒤 즉시 종료합니다 (단일 인스턴스)
- Win32 API로 포커스 변화를 실시간 감지하여 알림을 자동으로 닫습니다
- `--pid`에서 프로세스 트리를 거슬러 올라가 알림을 띄운 터미널 창을 찾습니다
- [Orca](https://orca.computer)처럼 모든 세션을 창 하나에 담는 앱은 프로세스 트리로 창을 찾을 수 없어, Orca가 기록하는 활성 탭을 읽어 탭 단위로 처리합니다. 지금 보고 있는 탭이면 알림을 생략하고, 그 탭으로 돌아가면 해당 알림만 닫히며, 알림을 누르면 그 탭까지 전환됩니다
...
- 기본으로 **Stop**(작업 완료)과 **Notification**(권한 요청) 훅이 등록됩니다. 더 세밀한 커스터마이즈는 서버의 `~/.claude/settings.json` 을 직접 편집하면 됩니다.

해제는 `agent-toast-send uninstall` 한 줄이면 됩니다. agent-toast 관련 훅만 제거하고 다른 훅은 보존합니다.
...
- 전송되는 데이터는 이벤트·소스별 누적 카운터와 설치 시 생성되는 랜덤 ID가 전부입니다
- 호스트명, 파일 경로, 메시지 내용 같은 개인 식별 정보는 **보내지 않습니다**
- 설정 → 통계 탭의 **익명 통계 공유** 토글로 끌 수 있습니다

## 🤔 왜 커스텀 알림 창인가요?

OS 네이티브 토스트는 알림을 띄우면 거기서 역할이 끝납니다. Agent Toast의 알림은 터미널로 돌아가는 길까지 책임집니다.

<p align="center">
  <img src=".github/media/toast.png" width="452" alt="Agent Toast 알림 토스트">
</p>

- 알림을 누르면 알림을 띄운 바로 그 터미널 창이 활성화됩니다
- 터미널로 포커스가 돌아오면 알림이 알아서 닫힙니다
- 이미 그 터미널을 보고 있으면 애초에 알림을 띄우지 않습니다

이런 동작은 어떤 창이 알림을 띄웠는지 알아야 가능해서, 네이티브 토스트 대신 전용 알림 창을 사용합니다.
